...
$GraphCli = Join-Path $Root 'cli\arnes-graph.ps1'
$work = Join-Path $Root ('.arnes-graph-test-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    try {
        ...
        Write-Output 'PASS arnes-graph: add/neighbors/path/stats contra SQLite real'
        exit 0
