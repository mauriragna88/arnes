...
.EXAMPLE
.\arnes-graph.ps1 add -NodeA Login.tsx -NodeB zod -Relation uses -Agent vivi
.\arnes-graph.ps1 query -Node Login.tsx
.\arnes-graph.ps1 neighbors -Node Login.tsx -Depth 2
.\arnes-graph.ps1 path -Start Login.tsx -End ansem
.\arnes-graph.ps1 stats
#>
[CmdletBinding()]
param(
...
switch ($Command) {
    'add' {
        ...
        $tmp = Join-Path $env:TEMP ("arnes-graph-" + [guid]::NewGuid().ToString('N') + ".json")
