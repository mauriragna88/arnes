...
1. **RECALL (TURN 0.5)**: leer contexto histórico
   `arnes-memory.ps1 search -Query "<keywords del quest>"` — quests similares
   `arnes-graph.ps1 stats` — mapa del proyecto
   Leer sam-digest.json (puente inter-quest) + shared-blackboard.json
...
4. **Delegar**: task() a los agentes del party (cada uno usa su modelo asignado en config.json)
...
7. **GUARDAR**: quest en arnes.db + digest de la sesión
   `arnes-memory.ps1 quest -Json '{"description":"...","result":"PASS","tokens_used":N}'`
...
## Memoria
- **Antes**: `search -Agent atlas` (quest-history, decisions, party-results)
- **Después**: `save -Agent atlas` (quest-history, decisions, digest)
